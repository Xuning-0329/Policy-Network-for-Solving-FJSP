...
#pb_instance:{'machinesNb': machinesNb, 'jobs': jobs}
# os_ms  n*(OS,MS)
...
#######################
# population  n*(OS,MS)
#parameters:{'machinesNb': machinesNb, 'jobs': jobs}
def elitistSelection(population, parameters):
    keptPopSize = int(config.pr * len(population)) #pr = 0.005 #len(popluation)=400
    sortedPop = sorted(population, key=lambda cpl: timeTaken(cpl, parameters))
    # print(sortedPop[:keptPopSize])
